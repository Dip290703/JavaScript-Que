  ...
  return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-20 pt-10 border-t'>
      <div className="min-w-60">
        <p className='my-2 text-xl flex items-center cursor-pointer gap-2' onClick={() => setShowFilters((prev)=> !prev)}>FILTERS
          <img src={assets.dropdown_icon} alt="drop" className={`h-3 sm:hidden ${showFilters ? "rotate-90" : ""}`} 
       />
        </p>
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6'>
         {
          filteredProducts.map((product,index)=>(
            <ProductItem  id={product._id} key={index} name={product.name} price={product.price} image={product.image}/>
          ))
         }
        </div>
      </div>
